...
export function getPrincipleMasterLevel(criteria: WCAGCriteria[], principle: Principle): number {
  const principleCriteria: WCAGCriteria[] = getPrincipleCriteria(
    criteria,
    getPrincipleNum(principle)
  )
...
export function getMasterProgress(
  masterLevel: number,
  criteria: WCAGCriteria[]
): { progress: number; total: number } {
  let progress: number = getTotalSolvesForMasterLevel(masterLevel, criteria)
  const total: number =
    criteria.length * (masterLevel < maxMasterLevel ? masterLevel + 1 : maxMasterLevel)
